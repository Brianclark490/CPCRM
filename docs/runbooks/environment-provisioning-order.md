...
The Static Web App must be connected to the GitHub repository so that GitHub Actions can deploy the frontend automatically.

```bash
az staticwebapp create \
  --name swa-cpcrm-dev \
  --resource-group rg-cpcrm-dev \
  --source https://github.com/Brianclark490/CPCRM \
  --location eastus \
  --branch develop \
...
> **Note:** The `--login-with-github` flag opens a browser window to authorise the GitHub integration. Ensure you are logged in to GitHub with appropriate repository access.

The deployment token generated by this step should be stored as a GitHub Actions secret:
...
| Static Web App CI not triggering | GitHub token expired or wrong branch | Re-run Step 5 to regenerate token |
