...
/** Extract the GET / handler registered on the router. */
function getHandler() {
  const layer = (healthRouter as unknown as { stack: Array<{ route: { path: string; methods: { get?: boolean }; stack: Array<{ handle: Function }> } }> }).stack
    .find((l) => l.route?.path === '/' && l.route.methods.get);
  if (!layer) throw new Error('GET / handler not found on healthRouter');
